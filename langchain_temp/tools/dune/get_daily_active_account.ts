...
export async function get_daily_active_account (): Promise<string> {
  try {
    ...
    if (
      ...
      return`Aptos daily active account is ${firstRow.daily_active_accounts} `
    } else {
      return `sorry i cant get Aptos daily active account , something  error `
    }
  } catch (error: unknown) {
